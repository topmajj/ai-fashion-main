...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { image, mask, gender, country, age, pose } = await req.json()

    // Validate required fields
    if (!image || !mask || !gender || !country || !age || !pose) {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: `Failed to generate 1-piece model: ${errorText}` }, { status: response.status })
    }
    ...
    console.error("Error in 1-piece model API route:", error)
