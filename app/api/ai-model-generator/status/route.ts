...
export async function POST(req: Request) {
  ...
  console.log("AI Model Generator Status API called")
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json({ status: "completed", imageUrl: generationData.image_url }, { status: 200 })
    ...
    try {
      ...
      const imageUrl = await thenewblackResponse.text()

      if (imageUrl && !imageUrl.includes("error") && !imageUrl.includes("pending") && imageUrl !== "Processing...") {
        // Valid image URL received
        ...
        return NextResponse.json({ status: "completed", imageUrl }, { status: 200 })
      } else if (imageUrl && imageUrl.includes("error")) {
        throw new Error(`Image generation failed: ${imageUrl}`)
      } else {
        // Still processing
