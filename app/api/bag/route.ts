...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { bag, gender, negative } = await req.json()
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: `Failed to generate bag image: ${errorText}` }, { status: response.status })
    }
    ...
    console.error("Error in bag API route:", error)
