...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { bracelet, gender, negative } = await req.json()
    ...
    formData.append("bracelet", bracelet)
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: `Failed to generate bracelet image: ${errorText}` },
    ...
    console.error("Error in bracelet API route:", error)
    return NextResponse.json(
      {
