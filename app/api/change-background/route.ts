...
export async function POST(req: Request) {
  console.log("change-background API route called")
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: `Failed to change background: ${errorText}` }, { status: response.status })
    }
    ...
    console.error("Error in change-background API route:", error)
