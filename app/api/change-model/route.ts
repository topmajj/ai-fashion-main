...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: `Failed to change model: ${errorText}` }, { status: response.status })
    }
    ...
    console.error("Error in change model API route:", error)
