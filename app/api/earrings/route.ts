...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { earrings, gender, negative } = await req.json()
    ...
    formData.append("earrings", earrings)
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: `Failed to generate earrings image: ${errorText}` },
    ...
    console.error("Error in earrings API route:", error)
    return NextResponse.json(
      {
