...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { shoes, gender, negative } = await req.json()
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: `Failed to generate shoes image: ${errorText}` }, { status: response.status })
    }
    ...
    logError(error, "shoes API route")
