...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { sunglasses, gender, negative } = await req.json()

    // Validate required fields
    if (!sunglasses || !gender) {
    ...
    formData.append("sunglasses", sunglasses)
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: `Failed to generate sunglasses image: ${errorText}` },
    ...
    console.error("Error in sunglasses API route:", error)
    return NextResponse.json(
      {
