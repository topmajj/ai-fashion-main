...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { watch, gender, negative } = await req.json()
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: `Failed to generate watch image: ${errorText}` }, { status: response.status })
    }
    ...
    console.error("Error in watch API route:", error)
