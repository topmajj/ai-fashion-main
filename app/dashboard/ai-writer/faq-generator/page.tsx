...
export default function FAQGenerator() {
  ...
  const [history, setHistory] = useState<FAQItem[]>([])
  ...
  const fetchHistory = async () => {
    try {
      ...
      const response = await fetch(`/api/ai-writer/faq-generator?userId=${user.id}&page=${currentPage}`)
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      console.log(`handleGenerate: Generating FAQ for topic: ${topic}, userId: ${user.id}`)
      const response = await fetch("/api/ai-writer/faq-generator", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ topic, context, numberOfQuestions, userId: user.id }),
      })
      ...
      setGeneratedFAQ(data.content)
      fetchHistory()
    } catch (error) {
      console.error("Error generating FAQ:", error)
      toast({
        title: "Error",
        description: `Failed to generate FAQ: ${error instanceof Error ? error.message : "Unknown error"}`,
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      console.log(`handleDelete: Deleting FAQ with id: ${id}`)
      const response = await fetch(`/api/ai-writer/faq-generator?id=${id}&userId=${user.id}`, {
        method: "DELETE",
      })
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to delete FAQ: ${JSON.stringify(errorData)}`)
      }
  ...
  return (
    <Layout>
      <div className="mb-8">
        <h1 className="text-3xl font-bold">FAQ Generator</h1>
        <p className="text-gray-600">Generate comprehensive FAQs for any topic</p>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Textarea
              ...
              placeholder="Provide any additional context or specific areas to cover"
              rows={3}
            />
          </div>
          <div>
      ...
      {generatedFAQ && (
        <Card className="mt-8 p-6">
          <h2 className="text-2xl font-bold mb-4">Generated FAQ</h2>
          <pre className="whitespace-pre-wrap text-sm">{generatedFAQ}</pre>
          <Button variant="outline" size="sm" onClick={() => handleCopy(generatedFAQ)} className="mt-4">
          ...
          <Card key={item.id} className="mb-4 p-4 bg-gray-50 dark:bg-gray-800">
            <div className="flex justify-between items-start mb-2">
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400">Questions: {item.number_of_questions}</p>
              </div>
